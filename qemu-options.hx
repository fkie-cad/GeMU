...
DEFHEADING(GEMU Options:)


DEF("symbolmapping", HAS_ARG, QEMU_OPTION_symbolmapping, \
    "-symbolmapping file  Give the symbolmapping for QEMU\n", QEMU_ARCH_ALL)
SRST
``-symbolmapping file``
    Get the symbolmapping for the used windows version
ERST

DEF("apidoc", HAS_ARG, QEMU_OPTION_apidoc, \
    "-apidoc file  Give the apidoc.json for QEMU\n", QEMU_ARCH_ALL)
SRST
``-apidoc file``
    Get the apidoc for the used windows version
ERST

DEF("syscalltable", HAS_ARG, QEMU_OPTION_syscalltable, \
    "-syscalltable file  Give the syscalltable.json for QEMU\n", QEMU_ARCH_ALL)
SRST
``-syscalltable file``
    Get the syscalltable
ERST

DEF("watchedprograms", HAS_ARG, QEMU_OPTION_watchedprograms, \
    "-watchedprograms name  Give one or more process names that should be looked out for. Separate multiple names with ','\n", QEMU_ARCH_ALL)
SRST
``-watchedprograms file``
    Give one or more process names that should be looked out for. Separate multiple names with ','
ERST

DEF("trackingmode", HAS_ARG, QEMU_OPTION_trackingmode, \
    "-trackingmode   Give the mode for GeMU tracking behavior\n", QEMU_ARCH_ALL)
SRST
``-trackingmode [syscall|basicblock|both]``
    Give the mode for GeMU tracking behavior.
    You can choose between breaking on every basic block or every syscall (and sysret).
    The both option should only be used for debugging GeMU.
    Please note that the -dotnet settig can introduce breaking on basic blocks even if -trackingmode is set to syscall.
ERST

DEF("dotnet", HAS_ARG, QEMU_OPTION_dotnet, \
    "-dotnet         Specify the dotnet tracking behavior\n", QEMU_ARCH_ALL)
SRST
``-dotnet [on|off|auto]``
    Specify the dotnet tracking behavior.
    'on' will cause breaks on every basic block to track potential dotnet unpacking.
    'auto' will enable bb breaks as soon as dotnet software is detected. If bb breaks are already enabled, this defaults to 'on'.
ERST

DEF("codecarver", 0, QEMU_OPTION_codecarving, \
"-codecarver    Activate the codecarving feature of GeMU.\n", QEMU_ARCH_ALL)
